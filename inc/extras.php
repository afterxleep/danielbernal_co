 ...
 * @return array
 */
function danielbernal_co_body_classes( $classes ) {
	...
	if ( danielbernal_co_has_header_image() ||
		 ( danielbernal_co_has_cover_image() && 1 == danielbernal_co_jetpack_featured_image_display() && ! $is_wc_product_page ) ) {
		$classes[] = 'has-header-image';
	}

	// Adds a gravatar-logo-disabled when gravatar display is disabled.
	if  ( false === get_theme_mod( 'danielbernal_co_display_gravatar', true ) ) {
		$classes[] = 'gravatar-logo-disabled';
	}

	return $classes;
}
add_filter( 'body_class', 'danielbernal_co_body_classes' );
 ...
 * @return array
 */
function danielbernal_co_post_class( $classes ) {
...
add_filter( 'post_class', 'danielbernal_co_post_class' );


/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function danielbernal_co_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'danielbernal_co_pingback_header' );
